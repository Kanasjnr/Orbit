// Zombienet script: Alice deposits into the oDOT vault on the Orbit parachain (charlie RPC).
		...
		api.tx.odot
			...
			.signAndSend(alice, (result) => {
				if (result.status.isFinalized) {
					...
					reject(new Error("odot.deposit failed"));
				}
			})
			.catch(reject);
	});

	const shares = await api.query.odot.shares(alice.address);
