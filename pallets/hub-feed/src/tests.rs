...
fn nomination_reward_credits_odot_sink() {
	new_test_ext().execute_with(|| {
		assert_ok!(HubFeed::report_nomination_reward(feed_origin(), eid(1), 10, 50));
...
fn self_stake_reward_credits_edot_sink() {
	new_test_ext().execute_with(|| {
		assert_ok!(HubFeed::report_self_stake_reward(feed_origin(), eid(2), 11, 70));
...
fn slash_hits_edot_only() {
	new_test_ext().execute_with(|| {
		assert_ok!(HubFeed::report_slash(feed_origin(), eid(3), 12, 40));
...
#[test]
fn slash_over_cap_fails_and_does_not_dedup() {
	new_test_ext().execute_with(|| {
		set_slash_cap(25);
		assert_noop!(
			HubFeed::report_slash(feed_origin(), eid(4), 12, 100),
			DispatchError::Other("SlashExceedsSlashable")
		);
		assert_eq!(slashed(), 0);
		assert!(!ProcessedHubEvents::<Test>::contains_key(eid(4)));
	});
}
...
fn duplicate_hub_event_rejected() {
	new_test_ext().execute_with(|| {
		assert_ok!(HubFeed::report_nomination_reward(feed_origin(), eid(5), 1, 10));
		assert_noop!(
			HubFeed::report_nomination_reward(feed_origin(), eid(5), 1, 10),
...
fn non_feed_origin_rejected() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			HubFeed::report_nomination_reward(RuntimeOrigin::root(), eid(7), 1, 10),
			DispatchError::BadOrigin
		);
	});
}
